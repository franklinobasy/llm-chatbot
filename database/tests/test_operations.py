...
def test_add_prompt_to_conversation(test_collection):
    ...
    result = add_prompt_to_conversation(
        user_id, conversation_id, prompt, collection=test_collection
    )
    assert result
...
def test_get_prompts_from_conversation(test_collection):
    ...
    add_prompt_to_conversation(
        user_id, conversation_id, prompt1, collection=test_collection
    )
    add_prompt_to_conversation(
        user_id, conversation_id, prompt2, collection=test_collection
    )

    prompts = get_prompts_from_conversation(
        user_id, conversation_id, collection=test_collection
    )
